...
package GUI is
   --  Note
   --  A global lock is held on the GUI when GTK events are being processed
   --  and image data is being updated because Tasks.Download.Download_Type
   --  performs asynchronous updates from a different thread.
   ...
   --  Unlocks the GUI, calls the subprogram, and then locks the GUI again It
   --  is unsafe to call subprograms which themselves rely on the GUI's state
   --  from within an Unsafe_Subprogram_Type
   procedure Locked_Wrapper (Unsafe_Subprogram : Unsafe_Subprogram_Type) with
     Inline;
   --  A simple wrapper which perform an iteration of the Main Loop, locking
   --  the GUI first and then unlocking it afterwrads
