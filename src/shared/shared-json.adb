...
package body Shared.JSON is
	...
	procedure Wait_Until_Key (Reader : in out JSON_Simple_Pull_Reader; Key : String) is
	begin
		while not At_End (Reader) loop
			Read_Next (Reader);
			if Event_Kind (Reader) = Key_Name and then VS2S (Key_Name (Reader)) = Key then
				return;
			end if;
		end loop;
		raise Program_Error;
	...
	procedure Wait_Until_Event (Reader : in out JSON_Simple_Pull_Reader; Event : JSON_Event_Kind) is
	begin
		while not At_End (Reader) loop
			Read_Next (Reader);
