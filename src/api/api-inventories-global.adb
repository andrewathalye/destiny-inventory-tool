...
package body API.Inventories.Global is
	...
	begin
		...
		Modified_Item.Bucket_Location := Manifest.Tools.General; -- The item is now in the Vault
		Modified_Item.Bucket_Hash := Manifest.Tools.General'Enum_Rep; -- ^^
		Modified_Item.Transfer_Status := Profiles.Can_Transfer;

		Inventory.Inventory (Item.Default_Bucket_Location).Append (Modified_Item);
	end Add_Item;

	procedure Remove_Item (
		Inventory : in out Global_Inventory_Type;
		Item : Manifest.Tools.Item_Description) is
	begin
		for I in Inventory.Inventory (Item.Default_Bucket_Location).First_Index .. Inventory.Inventory (Item.Default_Bucket_Location).Last_Index loop
	...
	begin
		case Location is
			...
			when others =>
				-- Can't just check (General).Length because the items are organised
				-- by their default Bucket_Locations for convenience elsewhere
	...
	function Get_Item_Stack (
		Inventory : Global_Inventory_Type;
		Hash : Manifest.Manifest_Hash) return Manifest.Tools.Item_Description
	is begin
		for IDL of Inventory.Inventory loop
			for ID of IDL loop
	...
	function Vault_Inventory (Inventory : Global_Inventory_Type) return Item_Description_List_Bucket_Location_Type_Array is (Inventory.Inventory);
