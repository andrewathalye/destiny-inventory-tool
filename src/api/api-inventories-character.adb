...
package body API.Inventories.Character is
	...
	is
		Character_Items : Item_Description_List_Bucket_Location_Type_Array renames Inventory.All_Character_Items (Character.Character_ID);
		Relevant_Items : Item_Description_List renames Character_Items (Item.Default_Bucket_Location);
	...
	is
		Character_Items : Item_Description_List_Bucket_Location_Type_Array renames Inventory.All_Character_Items (Character.Character_ID);
		Relevant_Items : Item_Description_List renames Character_Items (Item.Bucket_Location);
	begin
		...
		raise Program_Error with "API.Inventories.Character: Virtual remove failed";
	end Remove_Item;
	...
	procedure Equip_Item (
		...
		Item : Manifest.Tools.Item_Description)
	is
		Equipped_Items : Item_Description_Bucket_Location_Type_Array renames Inventory.All_Equipped_Items (Character.Character_ID);
	...
	is (Natural (Inventory.All_Character_Items (Character.Character_ID) (Location).Length));

	-- Access to Item Data
	function Character_Items (
		Inventory : Character_Inventory_Type;
		Character : Profiles.Character_Type) return Item_Description_List_Bucket_Location_Type_Array
	is (Inventory.All_Character_Items (Character.Character_ID));
	
	function Equipped_Items (
		Inventory : Character_Inventory_Type;
		Character : Profiles.Character_Type) return Item_Description_Bucket_Location_Type_Array
	is (Inventory.All_Equipped_Items (Character.Character_ID));
	...
	begin
		...
		for C of Profile.Characters loop
			...
			for I of Profile.Character_Inventories (C.Character_ID) loop
				declare
					D : constant Manifest.Tools.Item_Description := Manifest.Tools.Get_Description (M, I);
				begin
					Inventory.All_Character_Items (C.Character_ID) (D.Bucket_Location).Append (D);
				end;
			end loop;
			...
			for I of Profile.Character_Equipment (C.Character_ID) loop
				declare
					D : constant Manifest.Tools.Item_Description := Manifest.Tools.Get_Description (M, I);
				begin
					Inventory.All_Equipped_Items (C.Character_ID) (D.Bucket_Location) := D;	
				end;
			end loop;
