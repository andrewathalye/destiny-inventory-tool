					...
					when Start_Array => -- Memberships
						Read_Destiny_Memberships : loop
							...
							case Event_Kind (Reader) is
								when Start_Object =>
									...
									begin
										Read_GUIC : loop
											...
											case Event_Kind (Reader) is
												when Key_Name =>
													if VS2S (Key_Name (Reader)) = "LastSeenDisplayName" then
														Read_Next (Reader);
														GUIC.Last_Seen_Display_Name := VS2UB (String_Value (Reader));	
													elsif VS2S (Key_Name (Reader)) = "membershipType" then
														Read_Next (Reader);
														GUIC.Membership_Type := Bungie_Platform_Type'Enum_Val (As_Integer (Number_Value (Reader)));
													elsif VS2S (Key_Name (Reader)) = "membershipId" then
														Read_Next (Reader);
														GUIC.Membership_ID := VS2UB (String_Value (Reader));
													end if;
												when End_Object => exit Read_GUIC;
												when others => null;
											end case;
										end loop Read_GUIC;
								...
								when End_Array => exit Read_Destiny_Memberships;
								when others => null;
							end case;
						...
						Read_Bungie_Net_User : loop
							...
							case Event_Kind (Reader) is
								when Key_Name =>
									if VS2S (Key_Name (Reader)) = "membershipId" then
									...
									elsif VS2S (Key_Name (Reader)) = "context" then -- This contains nested objects
										Read_Next (Reader); -- Start_Object
										declare
										...
										begin
											while Nesting_Level /= 0 loop
												Read_Next (Reader);
												case Event_Kind (Reader) is
													when Start_Object => Nesting_Level := @ + 1;
													when End_Object => Nesting_Level := @ - 1;
													when others => null;
												end case;
											end loop;
										end;
									end if;
								when End_Object => exit Read_Bungie_Net_User;
								when others => null;
							end case;
